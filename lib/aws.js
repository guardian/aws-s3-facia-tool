...
export function setS3 (object) {
    s3 = object;
}

export function getObject (object, callback) {
...
export function headObject (object, callback) {
    const inCache = cache.key(object.Key);
    const cachedContent = inCache.get();
    if (cachedContent) {
        return callback(null, cachedContent);
...
export function listObjects (object, callback) {
...
export function listObjectVersions (object, callback) {
    const inCache = cache.key(object.Prefix);
    const cachedContent = inCache.get();
    if (cachedContent) {
        return callback(null, cachedContent);
