...
Front.prototype.trailIds = function (stage) {
    const uniques = {};
    this.collections.forEach(collection => {
        const config = this.collectionsConfiguration[collection._id];
        if (!config) {
            throw new Error('Missing collection content of front \'' + this.id + '\'');
        } else {
            return config.trailIds(stage).forEach(trail => uniques[trail] = true);
        }
    });
